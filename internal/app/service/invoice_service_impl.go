...
func NewInvoiceService(repo invoice.Repository) InvoiceService {
...
func (s *invoiceServiceImpl) CreateInvoice(ctx context.Context, req CreateInvoiceRequest) (*invoice.Invoice, error) {
	if len(req.Items) == 0 {
...
func (s *invoiceServiceImpl) GetInvoice(ctx context.Context, id string) (*invoice.Invoice, error) {
	if id == "" {
...
func (s *invoiceServiceImpl) GetInvoiceByPaymentAddress(ctx context.Context, address string) (*invoice.Invoice, error) {
	if address == "" {
		return nil, fmt.Errorf("%w: payment address cannot be empty", ErrInvalidRequest)
	}

	paymentAddr, err := invoice.NewPaymentAddress(address)
	if err != nil {
...
func (s *invoiceServiceImpl) ListActiveInvoices(ctx context.Context) ([]*invoice.Invoice, error) {
...
func (s *invoiceServiceImpl) MarkInvoiceAsViewed(ctx context.Context, id string) error {
	return s.transitionInvoice(ctx, id, func(inv *invoice.Invoice) error {
...
func (s *invoiceServiceImpl) transitionInvoice(
	ctx context.Context,
	id string,
	transition func(*invoice.Invoice) error,
) error {
	if id == "" {
		return fmt.Errorf("%w: invoice ID cannot be empty", ErrInvalidRequest)
	}
...
func (s *invoiceServiceImpl) createInvoiceItem(req CreateInvoiceItemRequest) (*invoice.InvoiceItem, error) {
	if req.Description == "" {
		return nil, errors.New("description cannot be empty")
	}

	// Parse unit price
	unitPrice, err := invoice.NewUSDTAmount(req.UnitPrice)
	if err != nil {
	...
	item, err := invoice.NewInvoiceItem(req.Description, unitPrice, quantity)
	if err != nil {

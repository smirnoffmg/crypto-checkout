...
func TestPaymentValueObjects(t *testing.T) {
	...
	t.Run("PaymentAddress", func(t *testing.T) {
		t.Run("NewPaymentAddress - valid address", func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, address, paymentAddr.String())
			require.Equal(t, shared.NetworkTron, paymentAddr.Network())
		})
	...
	t.Run("ConfirmationCount", func(t *testing.T) {
		...
		t.Run("IsGreaterThanOrEqual", func(t *testing.T) {
			...
			require.True(t, count.IsGreaterThanOrEqual(required3))
			require.True(t, count.IsGreaterThanOrEqual(required5))
			require.False(t, count.IsGreaterThanOrEqual(required6))
		})
	})
	...
	t.Run("PaymentAmount", func(t *testing.T) {
		t.Run("NewPaymentAmount - valid amount", func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, amount, paymentAmount.Amount())
			require.Equal(t, shared.CryptoCurrencyUSDT, paymentAmount.Currency())
			require.Equal(t, "100.00 USDT", paymentAmount.String())
		})
		...
		t.Run("NewPaymentAmount - invalid currency", func(t *testing.T) {
			...
			require.Error(t, err)
			require.Contains(t, err.Error(), "invalid cryptocurrency")
		})
	})
	...
	t.Run("NetworkFee", func(t *testing.T) {
		t.Run("NewNetworkFee - valid fee", func(t *testing.T) {
			...
			require.Equal(t, "10.00 BTC", networkFee.String())
		})
	...
	t.Run("PaymentTimestamps", func(t *testing.T) {
		t.Run("NewPaymentTimestamps", func(t *testing.T) {
			...
			require.Equal(t, detectedAt, timestamps.DetectedAt())
			require.Nil(t, timestamps.ConfirmedAt())
			require.WithinDuration(t, time.Now().UTC(), timestamps.CreatedAt(), time.Second)
			require.WithinDuration(t, time.Now().UTC(), timestamps.UpdatedAt(), time.Second)
		})
		...
		t.Run("SetConfirmedAt", func(t *testing.T) {
			...
			require.Equal(t, confirmedAt, *timestamps.ConfirmedAt())
		})
		...
		t.Run("SetUpdatedAt", func(t *testing.T) {
			...
			require.Equal(t, updatedAt, timestamps.UpdatedAt())
		})
	})
}
